...
def scan_io_management(sequence, initial_position):
    sorted_sequence = sorted(sequence)
    
    # Split the sequence based on initial head position and direction
    lower = [x for x in sorted_sequence if x < initial_position]
    higher = [x for x in sorted_sequence if x >= initial_position]
    
    new_sequence = []
    
    new_sequence = higher + [199] + lower[::-1]
...
def plot_sequence(numbers, seek_time, num_cylinders):
    ...
    plt.title('SCAN (Elevator) Algorithm')
...
sequence = scan_io_management(ref_string, starting_head)
