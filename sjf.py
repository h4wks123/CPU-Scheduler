...
def sjf_algo(processes):
    ...
    while processes:
        ...
        if temp:
            shortest_burst = min(temp, key=lambda p:p.burst_time)
            new.append(shortest_burst)
            processes.remove(shortest_burst)
            current_time += shortest_burst.burst_time
        else:
            current_time += 1     
...
print()
print("Shortest Job First CPU Scheduling Algorithm:")
print()
